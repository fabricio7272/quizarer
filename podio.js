...
fetch('http://127.0.0.1:1880/podioresposta')
  ...
  .then(texto => {
    try {
      ...
      // Ordena jogadores por pontos (decrescente) e tempo (crescente)
      jogadores.sort((a, b) => {
        if (b.pontos !== a.pontos) return b.pontos - a.pontos;
        return a.tempo - b.tempo;
      });
      
      // Seleciona a lista do ranking no HTML
      const lista = document.getElementById('ranking');
      lista.innerHTML = '';
      jogadores.forEach(({ nome, pontos, tempo }, i) => {
        const li = document.createElement('li');
        li.innerHTML =
          `<strong>${i + 1}º:</strong> ${nome} - ${pontos} pts - ${(tempo - 90).toFixed(2)} s`;
        lista.appendChild(li);
      });

      // Atualiza nomes do pódio nas posições 1, 2 e 3
      ...
      // Mostra erro caso JSON inválido
